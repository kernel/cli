...
func TestMacOSAppleScriptShellQuotesURLAtRuntime(t *testing.T) {
	...
	assert.Contains(t, script, `tell application "Terminal" to close connectorWindow saving no`)
	...
	assert.Contains(t, script, `/usr/bin/test -f " & quoted form of startedPath`)
	assert.Contains(t, script, `/usr/bin/test -f " & quoted form of donePath`)
	assert.Contains(t, script, `busy of connectorTab`)
	doneCheck := strings.Index(script, `/usr/bin/test -f " & quoted form of donePath`)
	idleCheck := strings.Index(script, `busy of connectorTab`)
	closeTab := strings.Index(script, `close connectorWindow saving no`)
	assert.Less(t, doneCheck, idleCheck)
	assert.Less(t, idleCheck, closeTab)
