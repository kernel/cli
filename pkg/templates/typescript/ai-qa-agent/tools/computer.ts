...
import type { ActionParams, BaseAnthropicTool, ToolDefinitionParams, ToolResult } from './types/computer';
...
export class ComputerTool implements BaseAnthropicTool {
  ...
  private convertKeyCombinationToKernel(combo: string): string {
    ...
    // Handle space-separated repeated keys (e.g., "Down Down Down")
    // Kernel API doesn't support multiple keys in sequence, so use only the first one
    if (combo.includes(' ')) {
      const parts = combo.trim().split(/\s+/);
      const firstKey = parts[0] || combo;
      return this.convertToKernelKey(firstKey);
    }
  ...
  async call(params: ActionParams): Promise<ToolResult> {
    ...
    if (action === Action.SCROLL) {
      ...
      const scrollDirectionRaw = scrollDirectionParam || kwargs.scroll_direction;
      const scrollAmountValue = scrollAmount || scroll_amount;

      if (!scrollDirectionRaw || typeof scrollDirectionRaw !== 'string' || !['up', 'down', 'left', 'right'].includes(scrollDirectionRaw)) {
        throw new ToolError(`Scroll direction "${scrollDirectionRaw}" must be 'up', 'down', 'left', or 'right'`);
      }
      const scrollDirection = scrollDirectionRaw as 'up' | 'down' | 'left' | 'right';
